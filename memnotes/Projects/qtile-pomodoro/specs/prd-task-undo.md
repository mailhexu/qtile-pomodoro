...
updated: 2026-09-09
feature: task-undo
related_architecture: [architecture-task-undo.md]
related_epics: []
---
...
2. With Todoist sync disabled, undo reverses the latest undoable action from the current Qtile session when that action is either:
...
6. If no undoable action exists, or Todoist sync is enabled, the command is a safe no-op.
7. A newly performed completion or move replaces the previous undo opportunity; undo itself does not create redo support.
8. The overlay identifies undo as local-only when Todoist sync is enabled.
...
- Undo for added tasks, task-title edits, Todoist-synced actions, or external JSON edits.
- Remote deletion/recreation semantics.
...
- Unit tests prove empty undo and sync-enabled undo are harmless, and a later mutation replaces the available undo action.
...
- Resolved 2026-09-09: Undo is local-only. It is unavailable while Todoist sync is enabled, so a refresh cannot overwrite the restored local state.
- `u` is the navigation-mode command because it is unclaimed by the current overlay keymap.
